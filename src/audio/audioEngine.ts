export class AudioEngine {
  ...
  public constructor() {
    ...
    this.engineDistortion.curve = makeDistortionCurve(160);
    ...
    this.windHighpass.frequency.value = 1000;
  ...
  public update(speedMetersPerSecond: number, isDrifting: boolean, isAccelerating = false, lateralSpeed = 0, deltaSeconds = 0.016, isBraking = false, isReversing = false, isNitroActive = false): void {
    ...
    const windTarget = speedRatio > 0.22 ? Math.pow((speedRatio - 0.22) / 0.78, 1.2) * 0.26 : 0;
    this.windGain.gain.linearRampToValueAtTime(windTarget, t + 0.12);
