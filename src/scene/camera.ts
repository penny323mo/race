  ...
  return {
    ...
    update(target: THREE.Vector3, heading: number, speedMetersPerSecond: number, isDrifting: boolean, deltaSeconds: number, isAirborne = false): void {
      ...
      const driftLateralTarget = isDrifting
        ? THREE.MathUtils.clamp(angularVelocity * -0.62 * speedRatio, -7.4, 7.4)
        : 0;
      ...
      const lookTarget = new THREE.Vector3()
        ...
        .add(new THREE.Vector3(0, THREE.MathUtils.lerp(1.5, 0.2, speedRatio), 0));

      const positionSmoothing = 1 - Math.exp(-dt * THREE.MathUtils.lerp(9.5, 2.50, speedRatio));
