...
export class Game {
  ...
  public async start(options: GameOptions = { mode: "ai-battle", soundEnabled: true, trackId: null }): Promise<void> {
    ...
    rendererBundle.scene.fog = new THREE.FogExp2(0x06080f, 0.0040);
    ...
    const emitSparks = (pos: THREE.Vector3, count: number): void => {
      for (let i = 0; i < count; i++) {
        ...
        sparks.push({ mesh, vx: (Math.random() - 0.5) * 22, vy: 4 + Math.random() * 8, vz: (Math.random() - 0.5) * 22, life: 0, maxLife: 0.34 + Math.random() * 0.46 });
      }
    };
