...
export class Game {
  ...
  public async start(options: GameOptions = { mode: "ai-battle", soundEnabled: true, trackId: null }): Promise<void> {
    ...
    rendererBundle.scene.fog = new THREE.FogExp2(0x06080f, 0.0030);
    ...
    const emitSparks = (pos: THREE.Vector3, count: number): void => {
      for (let i = 0; i < count; i++) {
        ...
        sparks.push({ mesh, vx: (Math.random() - 0.5) * 60, vy: 8 + Math.random() * 13, vz: (Math.random() - 0.5) * 60, life: 0, maxLife: 0.46 + Math.random() * 0.60 });
      }
    };
    ...
    const render = (): void => {
      ...
      if (gateFlashIdx >= 0 && gateFlashIdx < track.gateLights.length) {
        ...
        gl.intensity = THREE.MathUtils.lerp(18, 300, t);
      ...
      if (car.isDrifting && !wasDrifting && raceActive && driftFlashCooldown <= 0) {
        ...
        cameraRig.addShake(0.24 * speedRatioBloom);
